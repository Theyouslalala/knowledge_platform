...
class QueryExpander:
    ...
    async def expand(self, query: str, num_queries: int = 3) -> list[str]:
        ...
        prompt = (
            f"Generate {num_queries} different search queries "
            f"to help find information to answer: {query}\n\n"
...
class ContextualCompressor:
    ...
    async def compress(self, query: str, documents: list[RetrievalResult]) -> list[RetrievalResult]:
        ...
        prompt = (
            ...
            "Extract only the relevant information from these documents. "
            "Return the relevant passages with their document numbers.\n\n"
            f"Documents:\n{docs_text}"
        )
        ...
        try:
            ...
            if compressed and len(compressed) > 50:
                return [
                    RetrievalResult(content=compressed, score=1.0, metadata={}, source="compressed")
                ]
        except Exception:
            pass
...
class RAGPipeline:
    ...
    async def retrieve(
        self, query: str, top_k: int = 5, use_expansion: bool = True
    ) -> list[RetrievalResult]:
        if not self.retriever:
            if self.embedder:
                self.retriever = HybridRetriever(self.vector_store, self.embedder, self.bm25_index)
            else:
                return []
